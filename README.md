...
基于 Anima 与 ComfyUI 的本地随机生图工作台。它把批量生成、五类提示词随机池、模型与修复、LoRA、风格预设、收藏与历史记录集中在一个轻量 WebUI 中，并默认只连接本机服务。
...
本次升级在现有工作台上加入可验证的资源选择、动态修复链和风格复用能力，并保留原有提示词池、采样器、CLIP、VAE 与输出结构。
...
- **LoRA 子目录支持**：按安全的相对路径识别和搜索 LoRA，兼容唯一文件名的旧配置，并在同名歧义时要求重新选择。
- **模型与修复控制**：实时读取 ComfyUI 主模型和放大模型；高清修复、手部、NSFW、面部和眼睛 Detailer 可独立开关。
- **规范化画师收藏**：每个 `@画师` 独立解析、收藏和恢复，快捷入口收纳在默认折叠区域。
- **风格预设**：可创建、覆盖、重命名、删除和收藏完整风格快照，应用后仍可逐项调整。
...
LoRA 可按完整子目录路径搜索，并独立启用、调整顺序和强度；主模型、高清修复、四个 Detailer、常用尺寸、步数、CFG、正负提示词与固定画师集中在生成控制区。
...
- **画师收藏**：每个 `@画师` 独立收藏、取消和快捷追加；旧的逗号分隔数据会自动规范化。
- **LoRA 管理**：读取 ComfyUI 当前可用的 LoRA，支持安全子目录、完整路径搜索、顺序和强度控制。
- **模型与修复**：选择当前可用主模型和放大模型，独立控制高清修复及手部、NSFW、面部、眼睛 Detailer。
- **风格预设**：保存模型、LoRA、尺寸、采样参数、修复开关、高级提示词和画师，并按收藏优先、最近更新排序。
...
| `data/style_presets.json` | 用户自建风格预设；采用本地原子写入。 |
| `output/AnimaRandom/<日期>/<批次>/` | ComfyUI 生成的图片。 |
| `AnimaBasicV7-Random-WebUI.json` | 派生后的可视化工作流。 |
| `templates/` | ComfyUI API/UI 工作流模板。 |
| `sources/` | 原始工作流副本和源文件。 |

`data/`、`output/` 和本地测试产物默认由 `.gitignore` 排除。风格预设保存在本项目运行时数据中；收藏画师与收藏条目通过 ComfyUI 的 `Anima Tools` 接口保存，不会写入本项目的 Git 工作区。
...
当前测试覆盖资源与 LoRA 路径校验、动态修复工作流、风格预设、画师规范化、ComfyUI 客户端、批处理管理、历史记录、随机池、自定义项、模板下载和批量导入 API。
