...
| AstrBot/QQ 运输排除与 V7 原生发布门禁 | **完成**：运行时代码不依赖 `anima_natural.upstream`，不构造 Event/Context/PluginSettings；上游运输层明确排除 | **不适用**：V7 WebUI 不提供 QQ、群权限或 AstrBot 命令管理 | **完成**：`test_v7_native_gate.py` 同时验证当前源码与禁用模式反例；前端契约禁止旧 API | **完成**：最新完整门禁为 `245 passed, 102 subtests passed`；Ruff、compileall、全部 JS 语法、native gate 与 `git diff --check` 均通过 |
